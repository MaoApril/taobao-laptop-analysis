...
def plot_price_distribution_count():
    ...
    counts = df_tmp['价格区间'].value_counts()
...
def plot_price_distribution_sales():
    ...
    sales_by_price = df_tmp.groupby('价格区间', observed=False)['销量'].sum().reset_index()

    fig, ax = plt.subplots(figsize=(10, 6))
    patches, texts, autotexts = ax.pie(
        sales_by_price['销量'],
        labels=sales_by_price['价格区间'],
        autopct='%1.1f%%'
    )

    legend = plt.legend(patches, sales_by_price['价格区间'],
...
def plot_top10_origin():
    ...
    colors = cmap(np.linspace(0, 1, len(cities_top10)))

    plt.figure(figsize=(10, 6))
    bars = plt.bar(cities_top10['城市'], cities_top10['销量'], color=colors)

    for bar, height in zip(bars, cities_top10['销量']):
...
def plot_wordcloud():
    ...
    # 加载词云形状遮罩
    try:
        mask = np.array(Image.open('../data/computer.png'))
...
def plot_top15_brands():
    ...
    ax = plt.gca()
    bars = ax.bar(sales['品牌'], sales['销量'])

    for bar, height in zip(bars, sales['销量']):
